...
- [ ] `HabitCard`: layout tipo Today Dashboard:
  - status circle,
  - nombre + metadata,
  - score/streak,
  - completed state con line-through/check.
- [ ] `CategoryBadge`: tratamiento badge light/monochrome, Lexend labels.
- [ ] `CategoryPicker`: card grid light, active border primary.
- [ ] `FrequencySelector`: controles técnicos, active primary, border low contrast.
- [ ] `HabitForm`: spacing de secciones y labels con tokens.
- [ ] `MasteryBadge`: pill/border light.
- [ ] `LevelProgress`: bar/labels con tokens Stitch.
- [ ] `CheckInButton`: acciones primary/secondary y modal alineados a cards light.

### Validación

- [ ] `npx tsc --noEmit`.
- [ ] `npm test` si no hay errores de typecheck.
- [ ] No se ejecutó build.
